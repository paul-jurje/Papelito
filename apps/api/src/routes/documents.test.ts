...
describe('documents routes', () => {
  ...
  describe('GET /api/documents', () => {
    ...
    it('returns the user\'s documents sorted by updatedAt desc', async () => {
      ...
      const res = await authed.get('/api/documents');
      expect(res.status).toBe(200);
      const docs = res.body.documents as Array<{ id: number; title: string }>;
  ...
  describe('POST /api/documents', () => {
    ...
    it('creates a document with default title and empty ProseMirror content', async () => {
      ...
      expect(res.body.document.id).toBeTypeOf('number');
  ...
  describe('GET /api/documents/:id', () => {
    ...
    it('returns 400 for a non-numeric id', async () => {
      ...
      const res = await authed.get('/api/documents/not-a-number');
      expect(res.status).toBe(400);
    ...
    it('returns 404 for a non-existent document', async () => {
      ...
      const res = await authed.get('/api/documents/99999');
  ...
  describe('PATCH /api/documents/:id', () => {
    ...
    it('returns 400 for a non-numeric id', async () => {
      ...
      const res = await authed.patch('/api/documents/abc').send({ title: 'x' });
      expect(res.status).toBe(400);
    });
  ...
  describe('DELETE /api/documents/:id', () => {
    ...
    it('returns 400 for a non-numeric id', async () => {
      ...
      const res = await authed.delete('/api/documents/foo');
      expect(res.status).toBe(400);
    });

    it('returns 404 for a non-existent document', async () => {
      const { authed } = await registerAndSubscribe('delete-missing@example.com');
      const res = await authed.delete('/api/documents/99999');
      expect(res.status).toBe(404);
    });
